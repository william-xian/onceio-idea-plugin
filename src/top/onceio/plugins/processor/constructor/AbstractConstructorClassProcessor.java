...
public abstract class AbstractConstructorClassProcessor extends AbstractClassProcessor {
  ...
  private boolean validateVisibility(@NotNull PsiAnnotation psiAnnotation) {
    final String visibility = OnceIOProcessorUtil.getAccessVisibility(psiAnnotation);
  ...
  public boolean validateBaseClassConstructor(@NotNull PsiClass psiClass, @NotNull ProblemBuilder builder) {
    ...
    builder.addError("OnceIO needs a default constructor in the base class");
    return false;
  }
  ...
  protected Collection<PsiField> getAllNotInitializedAndNotStaticFields(@NotNull PsiClass psiClass) {
    ...
    for (PsiField psiField : psiClass.getFields()) {
      // skip fields named $
      boolean addField = !psiField.getName().startsWith(OnceIOUtils.ONCEIO_INTERN_FIELD_MARKER);
  ...
  public Collection<PsiField> getRequiredFields(@NotNull PsiClass psiClass) {
    ...
    for (PsiField psiField : getAllNotInitializedAndNotStaticFields(psiClass)) {
      ...
      if (null != modifierList) {
        ...
        final boolean isNonNull = PsiAnnotationSearchUtil.isAnnotatedWith(psiField, OnceIOUtils.NON_NULL_PATTERN);
  ...
  private PsiMethod createConstructor(@NotNull PsiClass psiClass, @PsiModifier.ModifierConstant @NotNull String modifier,
    ...
    OnceIOLightMethodBuilder constructorBuilder = new OnceIOLightMethodBuilder(psiClass.getManager(), getConstructorName(psiClass))
    ...
    if (!fieldNames.isEmpty()) {
      boolean addConstructorProperties = onceIOConfigDiscovery.getBooleanOnceIOConfigProperty(ConfigKey.ANYCONSTRUCTOR_ADD_CONSTRUCTOR_PROPERTIES, psiClass);
      if (addConstructorProperties || !onceIOConfigDiscovery.getBooleanOnceIOConfigProperty(ConfigKey.ANYCONSTRUCTOR_SUPPRESS_CONSTRUCTOR_PROPERTIES, psiClass)) {
    ...
    constructorBuilder.withAnnotations(OnceIOProcessorUtil.getOnX(psiAnnotation, "onConstructor"));
  ...
  private PsiMethod createStaticConstructor(PsiClass psiClass, @PsiModifier.ModifierConstant String methodModifier, String staticName, boolean useJavaDefaults, Collection<PsiField> params, PsiAnnotation psiAnnotation) {
    OnceIOLightMethodBuilder methodBuilder = new OnceIOLightMethodBuilder(psiClass.getManager(), staticName)
      .withContainingClass(psiClass)
      .withNavigationElement(psiAnnotation)
  ...
  @NotNull
  private LightTypeParameterBuilder createTypeParameter(OnceIOLightMethodBuilder method, int index, PsiTypeParameter psiClassTypeParameter) {
